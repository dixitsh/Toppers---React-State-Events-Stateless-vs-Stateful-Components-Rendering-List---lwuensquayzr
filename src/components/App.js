...
const list = [{ name: 'Goa', country: 'India' },
...
const App = () => {
  return (
    <div id="main">
      <ol>
        {
          list.filter((data) => data.country === 'India').filter((data) => data.name === 'Goa' || data.name === 'Darjeeling' || data.name === 'Lonavala').map((data, index) => {
            return (
              <li key={`location${index + 1}`}>{data.name}</li>
            )
          })
        }
      </ol>
